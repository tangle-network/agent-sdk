...
`CreateAgentEnvironmentInput.idempotencyKey` makes generic environment creation one retry-safe operation only when the provider advertises `environmentCreate.idempotency: "durable"`.
...
Providers that cannot prove those properties must reject keyed creates before allocation rather than expose process-local deduplication.
`environmentCreate.secretReferences: true` means `secrets` contains provider-resolved names, never inline values.
The existing `AgentEnvironmentProvider.create()` method carries this contract; it does not add a second create method.
