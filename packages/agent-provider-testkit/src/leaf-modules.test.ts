...
describe("testkit split leaf modules", () => {
  ...
  it("directly runs each conformance leaf through its fail-closed entry path", async () => {
    ...
    await expect(runPortableContextConformance({ name: "invalid", request: {} as never, rejectionRequest: {} as never, run: {} as never, acceptedAt: "invalid", plan: async () => ({}) as never, transfer: async () => ({}) as never, boundary: async () => null, continueNative: async () => ({}) as never, counters: () => ({ plans: 0, transfers: 0, freshSessions: 0, nativeContinuations: 0 }) })).rejects.toThrow();
