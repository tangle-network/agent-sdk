...
describe("retained cli-bridge sessions", () => {
  ...
  it("returns the bridge acknowledgement for exact interaction retries", async () => {
    ...
    if (!dispatched?.controlRef) throw new Error("interaction run identity was not returned");
    const command = {
  ...
  it("rejects interaction responses that omit or change the exact live run binding", async () => {
    ...
    if (!dispatched?.controlRef || !environment.respondToInteraction) {
    ...
    const command = {
      ...
      response: { id: "interaction-binding-question", outcome: "accepted" as const },
    };
    ...
    await expect(environment.respondToInteraction({
      ...command,
      binding: { ...command.binding, sessionId: "another-session" },
    })).rejects.toThrow(/must include this retained session/);
    await expect(environment.respondToInteraction({
      ...command,
      binding: { ...command.binding, runId: "another-run" },
