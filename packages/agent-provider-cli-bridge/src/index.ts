...
interface RetainedSessionState {
  ...
  cancelOperations: Map<string, {
    digest: AgentRunCancellationRequest["requestDigest"];
...
function createRetainedSession(
  ...
  const session: AgentSession & {
    ...
    result: async (resultOptions) => {
      const exactRef = historicalRetainedControlRef(
        context,
        state,
        requestedControlRef,
      );
      return collectRetainedResult(context, state, exactRef, undefined, resultOptions?.signal);
    },
    prompt: async (turn) => {
      rejectUnsupportedRetainedInput(context, turn);
      const attach = !hasTurnContent(turn) &&
...
async function retainedControlRef(
  ...
  state: RetainedSessionState,
  requestedControlRef?: AgentRunControlRef,
): Promise<ExactRetainedControlRef> {
  ...
  return exactRetainedControlRef(
    context,
    state,
    makeControlRef(
      context.name,
...
async function sendRetainedCancellation(
  ...
  if (existing) {
    ...
    return cancellationConflict(exactRequest, existing.digest);
  }
...
function cancellationConflict(
  request: AgentRunCancellationRequest,
  existingRequestDigest: AgentRunCancellationRequest["requestDigest"],
): AgentRunCancellationAcknowledgement {
...
async function respondToRetainedInteraction(
  ...
  if (parsed.binding.provider !== context.name) {
    throw new Error("cli-bridge interaction binding has the wrong provider");
  }
  ...
  if (
    !currentControlRef ||
    parsed.binding.runId !== currentControlRef.runId ||
    parsed.binding.executionId !== currentControlRef.executionId ||
    parsed.binding.requestDigest !== currentControlRef.requestDigest
  ) {
...
function interactionAcknowledgementMatchesCommand(
  ...
  return acknowledgement.operationId === command.operationId &&
    ...
    acknowledgement.binding.provider === command.binding.provider &&
    ...
    acknowledgement.binding.executionId === command.binding.executionId &&
    acknowledgement.binding.requestDigest === command.binding.requestDigest &&
