...
Pass `defaultCliBridgeCapabilities('pi')` as the provider capability document.
Alternatively, configure a `pi/...` default model so reconstruction derives the same capability document after restart.
