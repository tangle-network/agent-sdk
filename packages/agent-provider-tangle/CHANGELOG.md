...
- Updated dependencies [bcdd9aa]
- Updated dependencies [99c2a22]
  - @tangle-network/agent-interface@2.13.0

## 1.6.0

### Minor Changes

- 5a93f65: Bound file-mount and inline-resource content as payload, in bytes, instead of holding it to the control-plane string limit.
...
- 5a21744: A JSON bound refusal is a `JsonBoundError` — `name: "JsonBoundError"`, `code: "JSON_BOUND_VIOLATION"`, with the caller `label` and the whole `JsonBoundViolation` (rule, path, observed, limit) on the error — instead of a plain `Error` that only its message could describe.
...
- 957ab9d: Deliver a session event that arrives without a stable id instead of ending the stream.
...
- 5eb2f6c: Name the value that exceeded a JSON bound instead of throwing "value exceeds its JSON bound".
...
- 4943b30: Report an OpenCode execution's running token usage on each step frame, before the terminal receipt.
...
- ba7a885: Require an `@tangle-network/agent-interface` that bounds a turn prompt as content.
...
- 5f08e0e: Destroy a Tangle environment once, however many owners end it.
  ...
  `destroy()` now answers once per environment handle and every later caller joins that answer, for both the streaming environment and the exact-process one, which reaches the same guarded route. This is not a retry and hides no platform error: a delete that fails is not remembered, so the next caller issues a real one.
...
- Updated dependencies [646270c]
- Updated dependencies [d95d892]
...
- b0b718e: Use the Sandbox exact interactive handle for start, control, terminal attach, prompt, status, and stop operations.
